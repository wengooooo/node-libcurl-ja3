...
// read.
OPENSSL_EXPORT void AES_ctr128_encrypt(const uint8_t* in, uint8_t* out, size_t len,
...
OPENSSL_EXPORT void AES_ecb_encrypt(const uint8_t* in, uint8_t* out, const AES_KEY* key,
                                    const int enc);
...
OPENSSL_EXPORT void AES_cbc_encrypt(const uint8_t* in, uint8_t* out, size_t len, const AES_KEY* key,
...
OPENSSL_EXPORT void AES_ofb128_encrypt(const uint8_t* in, uint8_t* out, size_t len,
...
// but otherwise the buffers may not partially overlap. A partial overlap may
// overwrite input data before it is read.
OPENSSL_EXPORT void AES_cfb128_encrypt(const uint8_t* in, uint8_t* out, size_t len,
...
// -1.
OPENSSL_EXPORT int AES_wrap_key(const AES_KEY* key, const uint8_t* iv, uint8_t* out,
...
// |in_len| - 8 bytes to |out| and returns |in_len| - 8. Otherwise, it returns
// -1.
OPENSSL_EXPORT int AES_unwrap_key(const AES_KEY* key, const uint8_t* iv, uint8_t* out,
                                  const uint8_t* in, size_t in_len);
...
OPENSSL_EXPORT int AES_wrap_key_padded(const AES_KEY* key, uint8_t* out, size_t* out_len,
...
OPENSSL_EXPORT int AES_unwrap_key_padded(const AES_KEY* key, uint8_t* out, size_t* out_len,
                                         size_t max_out, const uint8_t* in, size_t in_len);
