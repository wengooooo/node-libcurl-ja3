...
typedef int (*OPENSSL_sk_call_cmp_func)(OPENSSL_sk_cmp_func, const void*, const void*);
typedef int (*OPENSSL_sk_call_delete_if_func)(OPENSSL_sk_delete_if_func, void*, void*);
...
OPENSSL_EXPORT void OPENSSL_sk_pop_free_ex(OPENSSL_STACK* sk,
                                           OPENSSL_sk_call_free_func call_free_func,
                                           OPENSSL_sk_free_func free_func);
OPENSSL_EXPORT size_t OPENSSL_sk_insert(OPENSSL_STACK* sk, void* p, size_t where);
...
OPENSSL_EXPORT void OPENSSL_sk_delete_if(OPENSSL_STACK* sk,
                                         OPENSSL_sk_call_delete_if_func call_func,
                                         OPENSSL_sk_delete_if_func func, void* data);
OPENSSL_EXPORT int OPENSSL_sk_find(const OPENSSL_STACK* sk, size_t* out_index, const void* p,
...
OPENSSL_EXPORT void OPENSSL_sk_sort(OPENSSL_STACK* sk, OPENSSL_sk_call_cmp_func call_cmp_func);
OPENSSL_EXPORT int OPENSSL_sk_is_sorted(const OPENSSL_STACK* sk);
OPENSSL_EXPORT OPENSSL_sk_cmp_func OPENSSL_sk_set_cmp_func(OPENSSL_STACK* sk,
                                                           OPENSSL_sk_cmp_func comp);
OPENSSL_EXPORT OPENSSL_STACK* OPENSSL_sk_deep_copy(const OPENSSL_STACK* sk,
...
OPENSSL_EXPORT OPENSSL_DEPRECATED void* sk_value(const OPENSSL_STACK* sk, size_t i);
...
OPENSSL_EXPORT OPENSSL_DEPRECATED void sk_pop_free_ex(OPENSSL_STACK* sk,
                                                      OPENSSL_sk_call_free_func call_free_func,
                                                      OPENSSL_sk_free_func free_func);
...
OPENSSL_EXPORT OPENSSL_DEPRECATED void sk_pop_free(OPENSSL_STACK* sk,
                                                   OPENSSL_sk_free_func free_func);
...
#define BORINGSSL_DEFINE_STACK_OF_IMPL(name, ptrtype, constptrtype)                                \
  ...
  typedef int (*sk_##name##_cmp_func)(constptrtype const*, constptrtype const*);                   \
  ...
  OPENSSL_INLINE void sk_##name##_call_free_func(OPENSSL_sk_free_func free_func, void* ptr) {      \
    ((sk_##name##_free_func)free_func)((ptrtype)ptr);                                              \
                                                                                                   ...
                                                                                                   \
  OPENSSL_INLINE void* sk_##name##_call_copy_func(OPENSSL_sk_copy_func copy_func,                  \
                                                  const void* ptr) {                               \
    return (void*)((sk_##name##_copy_func)copy_func)((constptrtype)ptr);                           \
  }                                                                                                \
                                                                                                   \
  OPENSSL_INLINE int sk_##name##_call_cmp_func(OPENSSL_sk_cmp_func cmp_func, const void* a,        \
                                               const void* b) {                                    \
    ...
    return ((sk_##name##_cmp_func)cmp_func)(&a_ptr, &b_ptr);                                       \
                                                                                                   ...
                                                                                                   \
  OPENSSL_INLINE int sk_##name##_call_delete_if_func(OPENSSL_sk_delete_if_func func, void* obj,    \
                                                     void* data) {                                 \
  ...
  OPENSSL_INLINE ptrtype sk_##name##_delete_ptr(STACK_OF(name) * sk, constptrtype p) {             \
    return (ptrtype)OPENSSL_sk_delete_ptr((OPENSSL_STACK*)sk, (const void*)p);                     \
  }                                                                                                \
                                                                                                   \
  OPENSSL_INLINE void sk_##name##_delete_if(STACK_OF(name) * sk, sk_##name##_delete_if_func func,  \
                                            void* data) {                                          \
    OPENSSL_sk_delete_if((OPENSSL_STACK*)sk, sk_##name##_call_delete_if_func,                      \
                         (OPENSSL_sk_delete_if_func)func, data);                                   \
  }                                                                                                \
                                                                                                   \
  OPENSSL_INLINE int sk_##name##_find(const STACK_OF(name) * sk, size_t * out_index,               \
                                      constptrtype p) {                                            \
    return OPENSSL_sk_find((const OPENSSL_STACK*)sk, out_index, (const void*)p,                    \
  ...
  OPENSSL_INLINE int sk_##name##_is_sorted(const STACK_OF(name) * sk) {                            \
    return OPENSSL_sk_is_sorted((const OPENSSL_STACK*)sk);                                         \
                                                                                                   ...
                                                                                                   \
  OPENSSL_INLINE sk_##name##_cmp_func sk_##name##_set_cmp_func(STACK_OF(name) * sk,                \
                                                               sk_##name##_cmp_func comp) {        \
    return (sk_##name##_cmp_func)OPENSSL_sk_set_cmp_func((OPENSSL_STACK*)sk,                       \
                                                         (OPENSSL_sk_cmp_func)comp);               \
  }                                                                                                \
                                                                                                   \
  OPENSSL_INLINE STACK_OF(name) * sk_##name##_deep_copy(const STACK_OF(name) * sk,                 \
                                                        sk_##name##_copy_func copy_func,           \
                                                        sk_##name##_free_func free_func) {         \
    return (STACK_OF(name)*)OPENSSL_sk_deep_copy(                                                  \
        (const OPENSSL_STACK*)sk, sk_##name##_call_copy_func, (OPENSSL_sk_copy_func)copy_func,     \
        sk_##name##_call_free_func, (OPENSSL_sk_free_func)free_func);                              \
  }                                                                                                \
                                                                                                   \
...
template <typename Stack>
using StackIterator = std::enable_if_t<StackTraits<Stack>::kIsStack, StackIteratorImpl<Stack>>;
...
// allocation failure.
template <typename Stack>
inline std::enable_if_t<!internal::StackTraits<Stack>::kIsConst, bool> PushToStack(
