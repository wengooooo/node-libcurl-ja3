...
// the |ERR_LIB_*| values.
OPENSSL_INLINE int ERR_GET_LIB(uint32_t packed_error) { return (int)((packed_error >> 24) & 0xff); }
...
OPENSSL_INLINE int ERR_GET_REASON(uint32_t packed_error) { return (int)(packed_error & 0xfff); }
...
// human-readable.
OPENSSL_EXPORT uint32_t ERR_get_error_line_data(const char** file, int* line, const char** data,
                                                int* flags);
...
OPENSSL_EXPORT uint32_t ERR_peek_error_line_data(const char** file, int* line, const char** data,
                                                 int* flags);
...
OPENSSL_EXPORT uint32_t ERR_peek_last_error_line_data(const char** file, int* line,
...
// |ERR_error_string|):
//
//   [thread id]:error:[error code]:[library name]:OPENSSL_internal:[reason string]:[file]:[line
//   number]:[optional string data]
//
...
OPENSSL_EXPORT void ERR_put_error(int library, int unused, int reason, const char* file,
                                  unsigned line);
