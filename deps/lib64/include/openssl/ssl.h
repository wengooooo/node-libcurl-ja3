...
OPENSSL_EXPORT void DTLSv1_set_initial_timeout_duration(SSL* ssl, uint32_t duration_ms);
...
OPENSSL_EXPORT int SSL_CREDENTIAL_set1_private_key(SSL_CREDENTIAL* cred, EVP_PKEY* key);
...
OPENSSL_EXPORT int SSL_CREDENTIAL_set1_signing_algorithm_prefs(SSL_CREDENTIAL* cred,
...
// and zero on error.
OPENSSL_EXPORT int SSL_CREDENTIAL_set1_cert_chain(SSL_CREDENTIAL* cred, CRYPTO_BUFFER* const* certs,
                                                  size_t num_certs);

// SSL_CREDENTIAL_set1_ocsp_response sets |cred|'s stapled OCSP response to
// |ocsp|. It returns one on success and zero on error.
OPENSSL_EXPORT int SSL_CREDENTIAL_set1_ocsp_response(SSL_CREDENTIAL* cred, CRYPTO_BUFFER* ocsp);
...
OPENSSL_EXPORT int SSL_CREDENTIAL_set1_signed_cert_timestamp_list(SSL_CREDENTIAL* cred,
...
OPENSSL_EXPORT void SSL_CREDENTIAL_set_must_match_issuer(SSL_CREDENTIAL* cred, int match);
...
OPENSSL_EXPORT const SSL_CREDENTIAL* SSL_get0_selected_credential(const SSL* ssl);
...
// |list|.
OPENSSL_EXPORT int SSL_CTX_set_signed_cert_timestamp_list(SSL_CTX* ctx, const uint8_t* list,
                                                          size_t list_len);
...
OPENSSL_EXPORT int SSL_set_signed_cert_timestamp_list(SSL* ctx, const uint8_t* list,
                                                      size_t list_len);
...
OPENSSL_EXPORT int SSL_CTX_set_ocsp_response(SSL_CTX* ctx, const uint8_t* response,
                                             size_t response_len);
...
OPENSSL_EXPORT size_t SSL_get_all_signature_algorithm_names(const char** out, size_t max_out);
...
// higher.
OPENSSL_EXPORT int SSL_CTX_set_signing_algorithm_prefs(SSL_CTX* ctx, const uint16_t* prefs,
                                                       size_t num_prefs);
...
// used. To disable them, set the minimum version to TLS 1.2 (default) or
// higher.
OPENSSL_EXPORT int SSL_set_signing_algorithm_prefs(SSL* ssl, const uint16_t* prefs,
                                                   size_t num_prefs);
...
OPENSSL_EXPORT int SSL_CTX_set_chain_and_key(SSL_CTX* ctx, CRYPTO_BUFFER* const* certs,
...
// objects are added as needed. Exactly one of |privkey| or |privkey_method|
// may be non-NULL. Returns one on success and zero on error.
OPENSSL_EXPORT int SSL_set_chain_and_key(SSL* ssl, CRYPTO_BUFFER* const* certs, size_t num_certs,
                                         EVP_PKEY* privkey,
                                         const SSL_PRIVATE_KEY_METHOD* privkey_method);
...
OPENSSL_EXPORT int SSL_CTX_use_PrivateKey_ASN1(int pk, SSL_CTX* ctx, const uint8_t* der,
                                               size_t der_len);
...
struct ssl_private_key_method_st {
  ...
  // progress on |ssl|.
  enum ssl_private_key_result_t (*decrypt)(SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out,
  ...
  // on |ssl|.
  enum ssl_private_key_result_t (*complete)(SSL* ssl, uint8_t* out, size_t* out_len,
                                            size_t max_out);
};
...
OPENSSL_EXPORT int SSL_CREDENTIAL_set_private_key_method(SSL_CREDENTIAL* cred,
                                                         const SSL_PRIVATE_KEY_METHOD* key_method);
...
// applicable versions.
OPENSSL_EXPORT const EVP_MD* SSL_CIPHER_get_handshake_digest(const SSL_CIPHER* cipher);
...
// caller. Future versions of BoringSSL may also return strings not in this
// list, so this does not apply if, say, sending strings across services.
OPENSSL_EXPORT size_t SSL_get_all_cipher_names(const char** out, size_t max_out);
...
// caller. Future versions of BoringSSL may also return strings not in this
// list, so this does not apply if, say, sending strings across services.
OPENSSL_EXPORT size_t SSL_get_all_standard_cipher_names(const char** out, size_t max_out);
...
OPENSSL_EXPORT void SSL_get0_signed_cert_timestamp_list(const SSL* ssl, const uint8_t** out,
                                                        size_t* out_len);
...
OPENSSL_EXPORT int SSL_get_tls_unique(const SSL* ssl, uint8_t* out, size_t* out_len,
                                      size_t max_out);
...
OPENSSL_EXPORT int SSL_export_keying_material(SSL* ssl, uint8_t* out, size_t out_len,
...
OPENSSL_EXPORT int SSL_SESSION_to_bytes_for_ticket(const SSL_SESSION* in, uint8_t** out_data,
                                                   size_t* out_len);
...
//
// WARNING: the returned data is not guaranteed to be well formed.
OPENSSL_EXPORT void SSL_SESSION_get0_ocsp_response(const SSL_SESSION* session, const uint8_t** out,
                                                   size_t* out_len);
...
OPENSSL_EXPORT size_t SSL_SESSION_get_master_key(const SSL_SESSION* session, uint8_t* out,
                                                 size_t max_out);
...
OPENSSL_EXPORT const uint8_t* SSL_SESSION_get0_id_context(const SSL_SESSION* session,
                                                          unsigned* out_len);
...
// should not be used.
OPENSSL_EXPORT int SSL_SESSION_set1_id_context(SSL_SESSION* session, const uint8_t* sid_ctx,
                                               size_t sid_ctx_len);
...
OPENSSL_EXPORT void SSL_SESSION_get0_ticket(const SSL_SESSION* session, const uint8_t** out_ticket,
                                            size_t* out_len);

// SSL_SESSION_set_ticket sets |session|'s ticket to |ticket|. It returns one on
// success and zero on error. This function may be useful in writing tests but
// otherwise should not be used.
OPENSSL_EXPORT int SSL_SESSION_set_ticket(SSL_SESSION* session, const uint8_t* ticket,
                                          size_t ticket_len);
...
// session ID context.
OPENSSL_EXPORT int SSL_CTX_set_session_id_context(SSL_CTX* ctx, const uint8_t* sid_ctx,
                                                  size_t sid_ctx_len);
...
OPENSSL_EXPORT void SSL_CTX_sess_set_get_cb(
    SSL_CTX* ctx,
    SSL_SESSION* (*get_session_cb)(SSL* ssl, const uint8_t* id, int id_len, int* out_copy));

// SSL_CTX_sess_get_get_cb returns the callback set by
// |SSL_CTX_sess_set_get_cb|.
OPENSSL_EXPORT SSL_SESSION* (*SSL_CTX_sess_get_get_cb(SSL_CTX* ctx))(SSL* ssl, const uint8_t* id,
...
OPENSSL_EXPORT int SSL_CTX_set_tlsext_ticket_key_cb(
    SSL_CTX* ctx, int (*callback)(SSL* ssl, uint8_t* key_name, uint8_t* iv, EVP_CIPHER_CTX* ctx,
...
struct ssl_ticket_aead_method_st {
  ...
  int (*seal)(SSL* ssl, uint8_t* out, size_t* out_len, size_t max_out_len, const uint8_t* in,
              size_t in_len);
...
OPENSSL_EXPORT SSL_SESSION* SSL_process_tls13_new_session_ticket(SSL* ssl, const uint8_t* buf,
                                                                 size_t buf_len);
...
// returns one on success and zero on failure.
OPENSSL_EXPORT int SSL_CTX_set1_group_ids(SSL_CTX* ctx, const uint16_t* group_ids,
...
// returns one on success and zero on failure.
OPENSSL_EXPORT int SSL_set1_group_ids(SSL* ssl, const uint16_t* group_ids, size_t num_group_ids);
...
// on success and zero on failure.
OPENSSL_EXPORT int SSL_CTX_set1_groups(SSL_CTX* ctx, const int* groups, size_t num_groups);

// SSL_set1_groups sets the preferred groups for |ssl| to be |groups|. Each
// element of |groups| should be a |NID_*| constant from nid.h. It returns one
// on success and zero on failure.
OPENSSL_EXPORT int SSL_set1_groups(SSL* ssl, const int* groups, size_t num_groups);
...
OPENSSL_EXPORT void SSL_CTX_set_custom_verify(
    SSL_CTX* ctx, int mode, enum ssl_verify_result_t (*callback)(SSL* ssl, uint8_t* out_alert));
...
OPENSSL_EXPORT void SSL_set_custom_verify(SSL* ssl, int mode,
                                          enum ssl_verify_result_t (*callback)(SSL* ssl,
                                                                               uint8_t* out_alert));
...
// |SSL_CTX_set_verify|.
OPENSSL_EXPORT int (*SSL_CTX_get_verify_callback(const SSL_CTX* ctx))(int ok,
...
OPENSSL_EXPORT int SSL_CTX_load_verify_locations(SSL_CTX* ctx, const char* ca_file,
                                                 const char* ca_dir);
...
OPENSSL_EXPORT void SSL_CTX_set_cert_verify_callback(
    SSL_CTX* ctx, int (*callback)(X509_STORE_CTX* store_ctx, void* arg), void* arg);
...
// higher.
OPENSSL_EXPORT int SSL_CTX_set_verify_algorithm_prefs(SSL_CTX* ctx, const uint16_t* prefs,
                                                      size_t num_prefs);
...
// are used. To disable them, set the minimum version to TLS 1.2 (default) or
// higher.
OPENSSL_EXPORT int SSL_set_verify_algorithm_prefs(SSL* ssl, const uint16_t* prefs,
                                                  size_t num_prefs);
...
OPENSSL_EXPORT void SSL_set0_CA_names(SSL* ssl, STACK_OF(CRYPTO_BUFFER) * name_list);
...
// convention.
OPENSSL_EXPORT int SSL_CTX_set_alpn_protos(SSL_CTX* ctx, const uint8_t* protos, size_t protos_len);
...
OPENSSL_EXPORT void SSL_get0_alpn_selected(const SSL* ssl, const uint8_t** out_data,
                                           unsigned* out_len);
...
OPENSSL_EXPORT int SSL_add_application_settings(SSL* ssl, const uint8_t* proto, size_t proto_len,
...
OPENSSL_EXPORT void SSL_get0_peer_application_settings(const SSL* ssl, const uint8_t** out_data,
                                                       size_t* out_len);
...
// useful.
typedef int (*ssl_cert_decompression_func_t)(SSL* ssl, CRYPTO_BUFFER** out, size_t uncompressed_len,
                                             const uint8_t* in, size_t in_len);
...
OPENSSL_EXPORT int SSL_CTX_add_cert_compression_alg(SSL_CTX* ctx, uint16_t alg_id,
...
OPENSSL_EXPORT void SSL_get0_next_proto_negotiated(const SSL* ssl, const uint8_t** out_data,
                                                   unsigned* out_len);
...
OPENSSL_EXPORT int SSL_select_next_proto(uint8_t** out, uint8_t* out_len, const uint8_t* peer,
...
OPENSSL_EXPORT void SSL_set_psk_client_callback(
    SSL* ssl, unsigned (*cb)(SSL* ssl, const char* hint, char* identity, unsigned max_identity_len,
...
OPENSSL_EXPORT int SSL_CREDENTIAL_set1_delegated_credential(SSL_CREDENTIAL* cred,
                                                            CRYPTO_BUFFER* dc);
...
OPENSSL_EXPORT int SSL_spake2plusv1_register(
    ...
    const uint8_t* password, size_t password_len, const uint8_t* client_identity,
    size_t client_identity_len, const uint8_t* server_identity, size_t server_identity_len);
...
OPENSSL_EXPORT SSL_CREDENTIAL* SSL_CREDENTIAL_new_spake2plusv1_client(
    ...
    size_t client_identity_len, const uint8_t* server_identity, size_t server_identity_len,
    uint32_t error_limit, const uint8_t* w0, size_t w0_len, const uint8_t* w1, size_t w1_len);
...
OPENSSL_EXPORT SSL_CREDENTIAL* SSL_CREDENTIAL_new_spake2plusv1_server(
    const uint8_t* context, size_t context_len, const uint8_t* client_identity,
    size_t client_identity_len, const uint8_t* server_identity, size_t server_identity_len,
    uint32_t rate_limit, const uint8_t* w0, size_t w0_len, const uint8_t* registration_record,
...
OPENSSL_EXPORT int SSL_set_quic_transport_params(SSL* ssl, const uint8_t* params,
                                                 size_t params_len);
...
OPENSSL_EXPORT int SSL_set_quic_early_data_context(SSL* ssl, const uint8_t* context,
                                                   size_t context_len);
...
OPENSSL_EXPORT int SSL_set1_ech_config_list(SSL* ssl, const uint8_t* ech_config_list,
...
OPENSSL_EXPORT void SSL_get0_ech_name_override(const SSL* ssl, const char** out_name,
                                               size_t* out_name_len);
...
// unparsable list.
OPENSSL_EXPORT void SSL_get0_ech_retry_configs(const SSL* ssl, const uint8_t** out_retry_configs,
...
OPENSSL_EXPORT int SSL_marshal_ech_config(uint8_t** out, size_t* out_len, uint8_t config_id,
...
OPENSSL_EXPORT int SSL_ECH_KEYS_marshal_retry_configs(const SSL_ECH_KEYS* keys, uint8_t** out,
                                                      size_t* out_len);
...
OPENSSL_EXPORT int SSL_get_ex_new_index(long argl, void* argp, CRYPTO_EX_unused* unused,
...
OPENSSL_EXPORT int SSL_CTX_get_ex_new_index(long argl, void* argp, CRYPTO_EX_unused* unused,
                                            CRYPTO_EX_dup* dup_unused, CRYPTO_EX_free* free_func);

OPENSSL_EXPORT int SSL_CREDENTIAL_set_ex_data(SSL_CREDENTIAL* cred, int idx, void* data);
OPENSSL_EXPORT void* SSL_CREDENTIAL_get_ex_data(const SSL_CREDENTIAL* cred, int idx);
OPENSSL_EXPORT int SSL_CREDENTIAL_get_ex_new_index(long argl, void* argp, CRYPTO_EX_unused* unused,
                                                   CRYPTO_EX_dup* dup_unused,
                                                   CRYPTO_EX_free* free_func);
...
// mismatch.
OPENSSL_EXPORT int SSL_request_handshake_hints(SSL* ssl, const uint8_t* client_hello,
                                               size_t client_hello_len, const uint8_t* capabilities,
...
OPENSSL_EXPORT void SSL_CTX_set_msg_callback(SSL_CTX* ctx,
                                             void (*cb)(int is_write, int version, int content_type,
...
OPENSSL_EXPORT void SSL_set_msg_callback(SSL* ssl, void (*cb)(int write_p, int version,
...
OPENSSL_EXPORT void (*SSL_CTX_get_keylog_callback(const SSL_CTX* ctx))(const SSL* ssl,
                                                                       const char* line);
...
OPENSSL_EXPORT int SSL_parse_client_hello(const SSL* ssl, SSL_CLIENT_HELLO* out, const uint8_t* in,
                                          size_t len);
...
OPENSSL_EXPORT void SSL_CTX_set_tmp_rsa_callback(SSL_CTX* ctx, RSA* (*cb)(SSL* ssl, int is_export,
                                                                          int keylength));
...
OPENSSL_EXPORT int SSL_get_shared_sigalgs(SSL* ssl, int idx, int* psign, int* phash,
...
OPENSSL_EXPORT void SSL_CTX_set_tmp_dh_callback(SSL_CTX* ctx,
                                                DH* (*cb)(SSL* ssl, int is_export, int keylength));
...
// caller. Future versions of BoringSSL may also return strings not in this
// list, so this does not apply if, say, sending strings across services.
OPENSSL_EXPORT size_t SSL_get_all_version_names(const char** out, size_t max_out);
...
OPENSSL_EXPORT void SSL_CTX_set_client_cert_cb(SSL_CTX* ctx, int (*cb)(SSL* ssl, X509** out_x509,
                                                                       EVP_PKEY** out_pkey));
...
OPENSSL_EXPORT int SSL_CTX_set1_curves(SSL_CTX* ctx, const int* curves, size_t num_curves);

// SSL_set1_curves calls |SSL_set1_groups|.
OPENSSL_EXPORT int SSL_set1_curves(SSL* ssl, const int* curves, size_t num_curves);
...
// |ctx|.
OPENSSL_EXPORT enum ssl_compliance_policy_t SSL_CTX_get_compliance_policy(const SSL_CTX* ctx);
...
// |ssl|.
OPENSSL_EXPORT enum ssl_compliance_policy_t SSL_get_compliance_policy(const SSL* ssl);
...
OPENSSL_EXPORT bool SSL_get_traffic_secrets(const SSL* ssl,
...
OPENSSL_EXPORT void SSL_CTX_set_aes_hw_override_for_testing(SSL_CTX* ctx, bool override_value);
...
OPENSSL_EXPORT void SSL_set_aes_hw_override_for_testing(SSL* ssl, bool override_value);
