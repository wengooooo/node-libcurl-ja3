...
using EnableIfContainer =
    std::enable_if_t<std::is_convertible_v<decltype(std::declval<C>().data()), T*> &&
...
class Span : private internal::SpanBase<const T> {
 public:
  ...
  constexpr Span(const C& container) : data_(container.data()), size_(container.size()) {}

  template <typename C, typename = internal::EnableIfContainer<C, T>,
            typename = std::enable_if_t<!std::is_const<T>::value, C>>
  constexpr explicit Span(C& container) : data_(container.data()), size_(container.size()) {}
...
template <typename C>
constexpr auto MakeConstSpan(const C& c) -> decltype(MakeConstSpan(c.data(), c.size())) {
