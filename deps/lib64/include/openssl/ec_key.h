...
OPENSSL_EXPORT int EC_KEY_set_public_key_affine_coordinates(EC_KEY* key, const BIGNUM* x,
                                                            const BIGNUM* y);
...
// zero.
OPENSSL_EXPORT int EC_KEY_oct2key(EC_KEY* key, const uint8_t* in, size_t len, BN_CTX* ctx);

// EC_KEY_key2buf behaves like |EC_POINT_point2buf|, except it encodes the
// public key in |key|.
OPENSSL_EXPORT size_t EC_KEY_key2buf(const EC_KEY* key, point_conversion_form_t form,
...
OPENSSL_EXPORT size_t EC_KEY_priv2oct(const EC_KEY* key, uint8_t* out, size_t max_out);
...
OPENSSL_EXPORT EC_KEY* EC_KEY_derive_from_secret(const EC_GROUP* group, const uint8_t* secret,
                                                 size_t secret_len);
...
OPENSSL_EXPORT int EC_KEY_get_ex_new_index(long argl, void* argp, CRYPTO_EX_unused* unused,
...
OPENSSL_EXPORT EC_GROUP* d2i_ECPKParameters(EC_GROUP** out, const uint8_t** inp, long len);
