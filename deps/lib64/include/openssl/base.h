...
#define OPENSSL_BEGIN_ALLOW_DEPRECATED __pragma(warning(push)) __pragma(warning(disable : 4996))
...
#define BORINGSSL_MAKE_UP_REF(type, up_ref_func) \
  inline UniquePtr<type> UpRef(type* v) {        \
    ...
    return UniquePtr<type>(v);                   \
                                                 ...
                                                 \
  inline UniquePtr<type> UpRef(const UniquePtr<type>& ptr) { return UpRef(ptr.get()); }
