...
class OPENSSL_EXPORT VerifyTrustStore {
 public:
  ...
  static std::unique_ptr<VerifyTrustStore> FromDER(std::string_view der_certs,
  ...
  static std::unique_ptr<VerifyTrustStore> FromDER(const std::vector<std::string_view>& der_certs,
                                                   std::string* out_diagnostic);
};
...
class OPENSSL_EXPORT CertPool {
 public:
  ...
  // certs in |der_certs|. In the event of a failure nullptr is returned and a
  // diagnostic string is placed in |out_diagnostic|
  static std::unique_ptr<CertPool> FromCerts(const std::vector<std::string_view>& der_certs,
                                             std::string* out_diagnostic);

 private:
  friend std::optional<std::vector<std::vector<std::string>>> CertificateVerifyInternal(
...
class OPENSSL_EXPORT CertificateVerifyOptions {
 public:
  ...
  CertificateVerifyOptions& operator=(const CertificateVerifyOptions&) = delete;
...
OPENSSL_EXPORT std::optional<std::vector<std::vector<std::string>>> CertificateVerifyAllPaths(
